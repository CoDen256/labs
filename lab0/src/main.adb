...
procedure Main is
   ...
   type TVector is array(Integer range <>) of Integer;
   type TMatrix is array(Integer range <>, Integer range <>) of Integer;
   type Vector is access TVector;
   type Matrix is access TMatrix;
   ...
   begin
      matrix_buf := new TMatrix(1..N, 1..N);
            for i in 1 .. N loop
               for j in 1 .. N loop
                  matrix_buf(i, j) := Cnst;
               end loop;
   ...
   begin
      ...
      function withoutMatrixChunk(MO: Matrix; chunkNum: Integer) return Matrix is
      M: Matrix(1..N, 1..MO'Last(2) - H);
   begin
      M := new TMatrix(1..N, 1..MO'Last(2) - H);
      --  for i in M'Range(1) loop
      --     for j in M'Range(2) loop
      --
      --        M(i, j) := MO(i, H + j);
   ...
   -- (N x 1) -> (H x 1)
   -- Get a chunk (of size H) of a vector based on the given chunk num
   function withoutVectorChunk(VO: Vector; chunkNum: Integer) return Vector is
         V: Vector(1..VO'Last(1)-H);
   begin
      --  V := new TVector(1..VO'Last(1)-H);
      --  for i in V'Range(1) loop
      --     V(i) := VO(H + i);
      --     end loop;
         return VO;
      end withoutVectorChunk;


   -- Insert the given chunk(V) into the given Vector (VO) based on the given chunk num
   procedure insertVectorChunk(VO: Vector; V: Vector; chunkNum: Integer) is
   begin
         for i in 1 .. H loop
            VO(H * chunkNum + i) := V(i);
         end loop;
   ...
   begin
      Result := new TMatrix(Mat1'Range(1), Mat2'Range(2));
   ...
   begin
      Result := new TMatrix(Mat'Range(2), Mat'Range(1));
      for i in Mat'Range(1) loop
   ...
   begin
      Result := new TVector(Vec1'Range(1));
   ...
   begin
      Result := new TVector(Vec'Range(1));
      for i in Vec'Range(1) loop
   ...
   ---- 1. T1 is the head of the task chain  (Creating MX, B + Outputing the result A)
   ---- 2. TP is the tail of the task chain  (Creating Z,D,C,MR)
   ---- 3. T is an inner node within the task chain

   ---- Communication diagram
   ---- (For example, 8 tasks. T1 - head, TP - tail. T2 - next after the head. T6 the task before the last TP)

   ---- 1. [T1] ->     MX,B     -> [T2] ->     MX,B     -> ... -> [T6] ->     MX,B     -> [TP]   # T1 creats MX, B and propagates chainwise until TP     [-> = submit_MX_B]
   ---- 2. [T1] <-   Z,D,C,MR   <- [T2] <-   Z,D,C,MR   <- ... <- [T6] <-   Z,D,C,MR   <- [TP]   # TP creates Z,D,C,MR and propagates chainwise until T1 [<- = submit_Z_D_C_MR]
   ---- 3. [T1] ->      a0      -> [T2] ->  min(a0,a1)  -> ... -> [T6] ->  min(a5,a6)  -> [TP]   # T1 computes a0 and propagates. Each next task computes ai, receives a(i-1) and propagates min(ai, a(i-1)). TP has min(a0, a1...aP) in the end [-> = submit_ai]
   ---- 4. [T1] ->      b0      -> [T2] ->    b0 + b1   -> ... -> [T6] ->    b0 + b1   -> [TP]   # T1 computes b0 and propagates. Each next task computes bi, receives b(i-1) and propagates bi + b(i-1). TP has (b0+b1...bP) in the end. [-> = submit_bi]
   ---- 5. [T1] <-     a, b     <- [T2] <-     a, b     <- ... <- [T6] <-     a, b     <- [TP]   # TP has both min(a0, a1...aP) and (b0+b1...bP). TP Propagates both values chainwise to the downstream until T1 [<- = submit_a_b]
   ---- 6. [T1] <-      Ah      <- [T2] <-      Ah      <- ... <- [T6] <-      Ah      <- [TP]   # TP computes Ah, inserts it to the empty matrix (A) and propagates. Each next task computes Ah, inserts it to the matrix (A) received from the previous. T1 has whole matrix A in the end. [<- = submit_A]
   ...
   ---[T1] The type for the head task of the chain.
   task type T1 is
   ...
   --- The type of the reference to T1
   ...
   task type T is
      ...
      entry init(newNum: in Integer;  newTasks: in TaskStack; head : in T1Ref; tail: in TPRef);
   ...
   task body T is
      ...
      tasks: TaskStack(1..P-2);  -- the reference to the task chain (task list)
                       -- P-2 because T1 and TP are not in the chain (not in the task list)

      first: T1Ref;  -- the reference to the head of the chain (T1)
   ...
   begin
      ...
      accept init(newNum: in Integer;  newTasks: in TaskStack; head : in T1Ref; tail: in TPRef) do
         num := newNum;
         tasks := newTasks;
         first := head;
         last := tail;
      ...
      if (num = P - 2) then
         ...
         last.submit_MX_B(withoutMatrixChunk(MX,num), withoutVectorChunk(B, num));
      else
         -- Pass MX,B to our successor
         Tasks(num+1).submit_MX_B(withoutMatrixChunk(MX,num), withoutVectorChunk(B,num));
      end if;



      --- Waiting for Z,D,C,MR to be passed from this taks's successor (successor because in the opposite direction: from TP to T1)
      ...
      if (num = 1) then
         ...
         first.submit_Z_D_C_MR(withoutVectorChunk(Z,num), withoutVectorChunk(D,num), withoutVectorChunk(C,num), withoutMAtrixChunk(MR,num));
      else
         -- Pass it to the predecessor
         Tasks(num-1).submit_Z_D_C_MR(withoutVectorChunk(Z,num), withoutVectorChunk(D,num), withoutVectorChunk(C,num), withoutMAtrixChunk(MR,num));
      end if;
  ...
  task body T1 is
   ...
   begin
      ...
      Tasks(num+1).submit_MX_B(withoutMatrixChunk(MX,num), withoutVectorChunk(B, num)); -- Actually its always the Task with num=1, since we are the head.
   ...
   begin
      ...
      ---
      -- Waiting for the MX, B to come up to us from the T1(head)
     accept submit_MX_B(newMX: in Matrix; newB: in Vector) do
         MX := newMx;
         B := newB;
      end submit_MX_B;
      ...
      -- Pass them downstream to the predecessor. Each task will pass it to its predecessor and so on until the head T1.
      Tasks(num-1).submit_Z_D_C_MR(withoutVectorChunk(Z,num), withoutVectorChunk(D,num), withoutVectorChunk(C,num), withoutMAtrixChunk(MR,num));
      ...
      MAh := computeMAh(MX, MR, num);
      Ah := computeAh(sumB, Z, D, MAh, minA, num);
      -- Compute Ah, we are the first one to start building up the matrix (A). Insert our chunk in the empty matrix and pass it downstream to the T1, where it will be fully completed.
      A := new TVector(1..N);
      insertVectorChunk(A, Ah, num);
   ...
   T1_HEAD: T1Ref := new T1;
   TP_TAIL: TPRef := new TP;


begin

   --  outputMatrix(withoutMatrixChunk(withoutMatrixChunk(createMatrix, 1),1));
   --  outputVector(withoutVectorChunk(withoutVectorChunk(createVector, 1),1));
   ...
   for i in Tasks'Range(1) loop
      Tasks(i).init(i, Tasks, T1_HEAD, TP_TAIL);
   end loop;
